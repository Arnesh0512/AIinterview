...
class RankingRequest(BaseModel):
    candidates_scores: List[List[Dict[str, Any]]]
    """
    [
        [ {candidate_id, raw_score, submitted_at}, ... ],  # Question 1
        [ {candidate_id, raw_score, submitted_at}, ... ],  # Question 2
        [ {candidate_id, raw_score, submitted_at}, ... ],  # Question 3
    ]
    """
...
def normalize_and_rank(candidates_scores):
    ...
    for i, candidate in enumerate(leaderboard):
        ...
        candidate["percentile"] = round(
            ((total_candidates - candidate["rank"]) / total_candidates) * 100,
            2
        )

        # Convert datetime to ISO string for JSON response
        if isinstance(candidate["latest_submission"], datetime):
            candidate["latest_submission"] = candidate["latest_submission"].isoformat()
...
@router.post("/contest")
def rank_candidates(data: RankingRequest):

    if not data.candidates_scores:
        raise HTTPException(status_code=400, detail="candidates_scores cannot be empty")

    try:
        leaderboard = normalize_and_rank(data.candidates_scores)
